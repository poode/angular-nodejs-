...
import { setDayOfWeek, toUTCDateTime } from './utils';
var numberSortPredicate = function (a, b) { return a - b; };
//TODO: move to kendo-date-math library
...
var weekInYear = function (date, weekStart) {
    ...
    days = Math.floor((date.getTime() - Date.UTC(year, 0, 1, -6)) / 86400000);
    return 1 + Math.floor(days / 7);
};
//TODO: move to util as it is used in frequencies too
...
/** @hidden */
export var months = function (date, end, rule) {
    var monthRules = rule.months, monthRuleValues = ruleValues(monthRules, date.getUTCMonth() + 1), changed = false;
...
/** @hidden */
export var monthDays = function (date, end, rule) {
    var monthLength, month, days, changed = false, normalize = function (monthDay) {
        if (monthDay < 0) {
    ...
    while (date <= end) {
        ...
        days = ruleValues(rule.monthDays, date.getUTCDate(), normalize);
        if (days === null) {
            return changed;
        }
        changed = true;
...
/** @hidden */
export var yearDays = function (date, end, rule) {
    var year, days, changed = false, normalize = function (yearDay) {
        if (yearDay < 0) {
    ...
    while (date < end) {
        ...
        days = ruleValues(rule.yearDays, dayInYear(date), normalize);
        if (days === null) {
            return changed;
        }
        changed = true;
...
/** @hidden */
export var weeks = function (date, end, rule) {
    var weekStart = rule.weekStart || Day.Monday, year, weekRuleValues, day, changed = false, normalize = function (week) {
        if (week < 0) {
            week = 53 + week;
        }
        return week;
    ...
    while (date < end) {
        weekRuleValues = ruleValues(rule.weeks, weekInYear(date, weekStart), normalize);
        ...
        if (weekRuleValues.length) {
            day = (weekRuleValues.sort(numberSortPredicate)[0] * 7) - 1;
...
/** @hidden */
export var weekDays = function (startDate, end, rule) {
    var ruleValue = rule.weekDays;
...
/** @hidden */
export var hours = function (date, end, rule) {
    var hourRules = rule.hours, startTime = rule._startTime, startHours = startTime.getUTCHours(), hourValues = ruleValues(hourRules, startHours), changed = false;
    if (hourValues !== null) {
        changed = true;
...
/** @hidden */
export var minutes = function (date, end, rule) {
    var minuteRules = rule.minutes, currentMinutes = date.getUTCMinutes(), minuteValues = ruleValues(minuteRules, currentMinutes), startHours = rule._startTime.getUTCHours(), changed = false;
...
/** @hidden */
export var seconds = function (date, _end, rule) {
    var secondRules = rule.seconds, startHours = rule._startTime.getUTCHours(), secondsValues = ruleValues(secondRules, date.getUTCSeconds()), dateMinutes = date.getUTCMinutes(), changed = false;
